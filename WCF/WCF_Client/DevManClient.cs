...
namespace WCF.WCF_Client
{
    ...
    public class DevManClient : ICallBack
    {
        ...
        private static ReaderWriterLockSlim syncer;                        // синхронизирует доступ к состоянию
        ...
        private DevManClient()
        {
            ...
            syncer = new ReaderWriterLockSlim(LockRecursionPolicy.SupportsRecursion);
        }
        ...
        public static ConnectionState State
        {
            get
            {
                if (syncer.TryEnterReadLock(100))
                {
                    try
                    {
            ...
            private set
            {
                if (syncer.TryEnterWriteLock(300))
                {
                    try
                    {
        ...
        public static void Connect()
        {
            try
            {
                if (State == ConnectionState.Disconnected || State == ConnectionState.Default)
                {
