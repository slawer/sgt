...
{
    /// <summary>
    /// Реализует параметр Расход на входе по Ходам Насоса с Аналогового сигнала
    /// </summary>
    public class P0109 : TParameter
    {
        ...
        public P0109(Guid p_identifier)
            : base(p_identifier, "P0109", "Расход на входе по Ходам Насоса с Аналогового сигнала")
        {
            simple = false;
        ...
        protected float CalculateFlow(float pump_movies, float pump_diameter)
        {
            ...
            if (_pairs != null)
            {
                ...
                {
                    int diametr = (int)Math.Round(pump_diameter);
                    int ideal_d = (int)Math.Round(_pair.Diameter);

                    if (ideal_d == pump_diameter)
                    {
                        flow = _pair.Flow;
                        break;
                    }
        ...
        public class IdealFlowPair
        {
            ...
            public IdealFlowPair(float _diameter, float _flow)
            {
                flow = _flow;
                diameter = _diameter;
