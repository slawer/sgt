...
namespace DeviceManager
{
    ...
    {
        private SGT.SgtApplication app = null;                 // констекст
        ...
        {
            app.Commutator.onUpdated -= Converter_OnComplete;
        ...
        private void dataGridViewCalibrationTable_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            ...
            oldValue = SGT.SgtApplication.ParseDouble(dataGridViewCalibrationTable[e.ColumnIndex, e.RowIndex].Value.ToString());
        }
        ...
        private void dataGridViewCalibrationTable_CellParsing(object sender, DataGridViewCellParsingEventArgs e)
        {
            if (Type.GetTypeCode(e.Value.GetType()) == TypeCode.String)
            {
                try
                {
                    newValue = SGT.SgtApplication.ParseDouble(e.Value.ToString());
