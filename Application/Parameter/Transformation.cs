...
namespace SGT
{
    ...
    public class Transformation
    {
        ...
        /// <summary>
        /// Возвращяет значение сигнала по его значению после обработки
        /// </summary>
        /// <param name="Result">Значение после обработки</param>
        /// <returns>Значение сигнала</returns>
        public float GetSignal(float Result)
        {
            bool blocked = false;
            try
            {
                if (mutex.WaitOne(100, false))
                {
                    ...
                    if (this.table[0].Result > s)
                    {
                        ind = 1;
                    }
                    else
                        if (this.table[LastIndex].Result < s)
                        {
                            ind = LastIndex;
                        }
                        else
                        {
                            for (int j = 1; j <= LastIndex; j++)
                            {
                                if ((this.table[j - 1].Result <= s) && (s <= this.table[j].Result))
                                {
                                    ind = j;
                                    break;
                                }
                    ...
                    // афинное преобразование сигнала в значение
                    double val = (s - this.table[ind].Shift) / this.table[ind].Multy;
                    return (float)val;
                }
            }
            finally
